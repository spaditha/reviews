    ...
    public void map(ImmutableBytesWritable rowKey, Result value, Context context)
      ...
      try {
		...
		if (overallrating == 1.0 || overallrating == 2.0) {
			mapToBucket(st.countTokens(), verified, true, context, one);
		} else if (overallrating == 4.0 || overallrating == 5.0) {
			mapToBucket(st.countTokens(), verified, false, context, one);
		}
...
// ####################################################################################################################################################
	...
	public static void mapToBucket(int tokens, boolean verified, boolean isNegativeReview, Context context, IntWritable one){
